...
exports.TEST = function (){
  ...
  if( Fs.copyFile(args.file_uri, destPath) ){
    ...
    response = qr_detection(args).postSync();

    var validate = validResponse.error === response.error ? true : false;
    validate = validate &&
      (validResponse.qr_messages[0] === response.qr_messages[0] ? true: false);
    validate = validate &&
      (validResponse.qr_centers[0].x === response.qr_centers[0].x ? true: false);
    validate = validate &&
      (validResponse.qr_centers[0].y === response.qr_centers[0].y ? true: false);
    validate = validate &&
      (validResponse.qr_centers.length === response.qr_centers.length ? true: false);
    validate = validate &&
      (validResponse.qr_messages.length === response.qr_messages.length ? true: false);
