 ...
 * @fileOverview
 *
 * [Visual-Localization] RAPP Platform front-end web service.
 *
...
const rosSrvName = "/rapp/rapp_visual_localization/localize";
...
function svcImpl(req, resp, ros) {
  ...
  rosMsg.image = req.files.file[0];
  rosMsg.pose_delta = req.body.pose_delta;
  rosMsg.id = req.body.id;
...
function parseRosbridgeMsg(rosbridge_msg) {
  ...
  response.belief = rosbridge_msg.belief;
  response.best_pose = rosbridge_msg.best_pose;
  response.error = rosbridge_msg.error;
  response.status = rosbridge_msg.status;
