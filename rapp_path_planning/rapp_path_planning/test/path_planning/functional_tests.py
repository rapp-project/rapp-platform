...
# test for
    def test_map_upload_error(self):
        node_name = 'rapp_map_server_functional_test1'
        rospack_map = rospkg.RosPack()
        upload_service = node_name+"/upload_map"
        try:
            ...
            with open(map_server_path+"/maps/523_m3.png", "rb") as imageFile:
                f = imageFile.read()
                file_bytes = bytes(f)
            up_prox = rospy.ServiceProxy(node_name+"/upload_map", MapServerUploadMapRosSrv)
            req = MapServerUploadMapRosSrvRequest()
    ...
    def test_map_upload(self):
    	...
    	package = 'rapp_map_server'
    	executable = 'rapp_map_server'
    	node_name = 'rapp_map_server_functional_test2'
        ...
        with open(map_server_path+"/maps/523_m3.png", "rb") as imageFile:
            f = imageFile.read()
            file_bytes = bytes(f)
        up_prox = rospy.ServiceProxy(upload_service, MapServerUploadMapRosSrv)
        req = MapServerUploadMapRosSrvRequest()
